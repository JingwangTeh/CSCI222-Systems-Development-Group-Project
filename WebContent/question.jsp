...
<script type="text/javascript">
	$(document).ready(function() {
						...
						$("body").on('click','[id^="answerAccept_"]',function() {
							var returnPath5 = window.location.pathname + window.location.search;
							var path = $("#path").val();
							window.location = path+"/question/acceptAnswer.action?returnPath="+returnPath5+"&answerId="+$(this).val()+"&question.id="+$("#questionId").val();
		...
		<div class="row ">
			<div class="col-md-8">
				...
				<table class="table" id="beforeComment">
					...
					<thead>
						...
						<s:if test="#request.question.repeatId!=0">
							There is a same question, <a href="${pageContext.request.contextPath }/question/scan.action?question.id=${request.question.repeatId }">click here to check the same question</a>
						</s:if>
					</thead>
					<tbody>
						<tr>
							...
							<ul class="media-list">
								<li class="media">
									...
									<div class="media-body" >
										...
										<s:if
											test="(#session.user.id==#request.question.ownerUserId) && (#request.question.acceptedAnswerId==null || #request.question.acceptedAnswerId=='')">
											<button type="button" class="btn btn-success btn-sm" id="answerAccept_<s:property value="#st.index" />" value="<s:property value="#as.id"/>" >
												<span class="glyphicon glyphicon-heart-empty-star"
													aria-hidden="true"></span> Accept this answer
											</button>
			...
			<div class="col-md-3 ">
				...
				<div class="jumbotron">
					<h2>Find Why?</h2>
					<p>
						<a class="btn btn-primary btn-lg" href="${pageContext.request.contextPath }/ask.jsp" role="button">ask
							question</a>
					</p>
				</div>
				...
				<br />
				<br />
				<s:if test="(null != #session.user ) && (#session.user.limitation>0)">
				<table width="27%" class="table table-striped">
					
					<thead>
						<tr>
							
							<form method="post" action="${pageContext.request.contextPath }/question/repeat.action">
								question id: <input name="questionId"/>
								<input name="question.id" value="${question.id }" class="hidden"/>
								<button type="submit" class="btn  btn-info pull-left  " id="fun_hot" value="1">repeat</button>
							</form>
							</div>
						</tr>
						<!-- <tr>
						<button type="button" class="btn  btn-info pull-left  " id="fun_hot" value="1">illegal</button>
						</tr> -->
					</thead>
				</table>
				<table width="27%" class="table table-striped">
					<thead>
						<tr>
						<form method="post" action="${pageContext.request.contextPath }/question/hidden.action">
							<input name="question.id" value="${question.id }" class="hidden"/>
							<button type="submit" class="btn  btn-info pull-left  " id="fun_hot" value="1">illegal</button>
						</form>
						</tr>
					</thead>
